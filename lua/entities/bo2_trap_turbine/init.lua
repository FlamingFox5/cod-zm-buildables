...
function ENT:Think()
	...
	if self:GetDestroyed() and self:GetActivated() then
...
function ENT:OnTakeDamage(dmginfo)
	...
	if self:Health() <= 0 then
		...
		local seq, dur = self:LookupSequence("death")

		ParticleEffectAttach( "bo2_turbine_death", PATTACH_POINT_FOLLOW, self, 1 )

		SafeRemoveEntityDelayed(self, self:SequenceDuration( seq ) - 0.2)
	else
		self:TurbineAnim()
...
function ENT:TurbineDecay()
	...
	if self.turbine_next_decay < CurTime() then
		...
		ParticleEffectAttach("bo2_turbine_flare", PATTACH_ABSORIGIN_FOLLOW, self, 1)
