...
function ENT:Initialize()
	if self.BuildableMinBounds and self.BuildableMaxBounds then
		self:PhysicsInitBox(self.BuildableMinBounds, self.BuildableMaxBounds, "metal")
	else
	...
	if self.BuildableMinBounds and self.BuildableMaxBounds then
		self:SetCollisionBounds(self.BuildableMinBounds, self.BuildableMaxBounds)
	end
	...
	if not util.IsInWorld( self:GetPos() ) and IsValid( ply ) then
...
function ENT:Use(ply)
	...
	timer.Simple(0, function()
		...
		if IsValid(wep) then
			...
			wep:SetClip1(math.Round(math.Clamp(health / max, 0, 1) * wep:GetStatL("Primary.ClipSize")))
