...
function SWEP:NotifyPlaceMessage( bIsDrone )
	...
	if IsValid(ply) and ply:IsPlayer() and IsFirstTimePredicted() and (not ply._TFA_LastJamMessage or ply._TFA_LastJamMessage < RealTime()) then
		ply:PrintMessage(HUD_PRINTCENTER, ( bIsDrone and "#tfa.msg.miscbuild.drone.hint" or "COULD NOT FIND AREA TO PLACE" ))
...
function SWEP:SetupPlaceable()
	...
	local vecFinal = ply.GetShootPos and ply:GetShootPos() or ply:EyePos()
	local qAngFwd = ply.GetAimVector and ply:GetAimVector():Angle() or ply:EyeAngles()
	qAngFwd = Angle(0, qAngFwd[2], 0)

	self.PlaceableSetup = { ["Position"] = vecFinal, ["Angle"] = qAngFwd, ["Direction"] = qAngFwd:Forward(), ["Normal"] = ply:GetForward(), ["Valid"] = !util.QuickTrace(vecFinal, qAngFwd:Forward() * 40, {self, ply}).Hit and (!ply.MaxisDrone or !IsValid(ply.MaxisDrone)) }
